    ...
    >
      ...
      <el-table-column min-width="300px" label="标题">
        <template slot-scope="{row}">
          <router-link :to="'/activity/edit/'+row.id + (isDraft ? '?isDraft=true' : '')" class="link-type">
            <span>{{ row.title }}</span>
          </router-link>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="Actions" width="120">
        <template slot-scope="scope">
          <router-link :to="'/activity/edit/'+scope.row.id + (isDraft ? '?isDraft=true' : '')">
...
export default {
  ...
  methods: {
    ...
    getDraft() {
      const list = JSON.parse(localStorage.getItem('activies') || '[]')
